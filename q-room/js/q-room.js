...
function renderAccountNotifList(items = []) {
  ...
  if(!list) return;
  // acct-user-section が非表示でも通知は描画しておく（表示切り替え後に反映される）
...
async function renderAccountPage() {
  ...
  // 通知リストは currentUserProfile に関わらず currentUser があれば常に表示
  if(currentUser) {
    try {
      const snap = await (db || firebase.database()).ref('notifications/' + currentUser.uid).once('value');
      const items = [];
      snap.forEach(c => items.push({ id: c.key, ...c.val() }));
      items.sort((a, b) => (b.ts || 0) - (a.ts || 0));
      console.log('[renderAccountPage] notif fetch OK, items=' + items.length);
      renderAccountNotifList(items);
    } catch(e) {
      console.error('[renderAccountPage] notif fetch ERROR:', e);
